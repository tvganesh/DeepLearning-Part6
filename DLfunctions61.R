...
###############TO delete########################
# Predict the output
predict2 <- function(parameters, X,hiddenActivationFunc='relu'){
    
    fwdProp <- forwardPropagationDeep(X, parameters,keep_prob=1,hiddenActivationFunc)
    predictions <- fwdProp$AL>0.5
...
# Plot a decision boundary
# This function uses ggplot2
plotDecisionBoundary2 <- function(z,retvals,hiddenActivationFunc,lr){
    xmin<-min(z[,1])
    xmax<-max(z[,1])
    ymin<-min(z[,2])
    ymax<-max(z[,2])
    ...
    colnames(grid) <- c('x1', 'x2')
    grid1 <-t(grid)
    q <-predict2(retvals$parameters,grid1,hiddenActivationFunc)
    q1 <- t(data.frame(q))
    q2 <- as.numeric(q1)
    grid2 <- cbind(grid,q2)
    colnames(grid2) <- c('x1', 'x2','q2')
